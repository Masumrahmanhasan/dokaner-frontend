...
const SectionsTwo = (props) => {
  const { general } = props;
  const section_two_title = getSetting(general, "section_two_title");
  const query_url = getSetting(general, "section_two_query_url");
  const query_type = getSetting(general, "section_two_query_type");
  ...
  useEffect(() => {
    if (_.isEmpty(products)) {
      loadSectionsProducts("section_two").then((response) => {
  ...
  return (
    <div className='container deal-section'>
      <div className='row mt-1 mb-0 mb-md-1 mb-md-1  pb-md-1'>
        <div className='col-6'>
          <h3 className='title title-sm'> {section_two_title || "..."}</h3>
        </div>
