...
const SectionsFour = (props) => {
  const { general } = props;
  const section_three_title = getSetting(general, "section_four_title");
  const query_url = getSetting(general, "section_four_query_url");
  const query_type = getSetting(general, "section_four_query_type");
  ...
  useEffect(() => {
    if (_.isEmpty(products)) {
      loadSectionsProducts("section_four").then((response) => {
