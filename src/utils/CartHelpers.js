...
export const getCartProductPrice = (product, rate = 15, ConfiguredItem = {}) => {
  let sellPrice = 0;
  if (!_.isEmpty(product)) {
    let Price = product.Price;
...
export const getUpdatedProductPrice = (totalQtyInCart, bulkPriceQuantity, product, rate, ConfiguredItem) => {
  let salePrice = 0;
  ...
  let reConfig = cartConfigured.map((mapItem) => {
    if (mapItem.id == product_id) {
      ...
      item_variations = mapItem.item_variations.map((config) => {
        if (config.id == existsConfigId) {
          return { ...config, refund_quantity: qty, refunded: qty * config.price };
        }
        return config;
      });
...
export const calculateDiscountAmount = (method, advance, general, methodString) => {
  ...
  const checkout_discount_fourth = getSetting(general, `checkout_${methodString}_discount_fourth`);
...
export const reFinalCart = (cartConfigured, chinaLocalShippingChargeLimit, chinaLocalShippingCharges) => {
